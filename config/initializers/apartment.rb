...
Apartment.configure do |config|
  ...
  #
  config.tenant_names = -> { Organization.select(:code).map(&:tenant_name) }
  config.excluded_models = %w[ActiveStorage::Attachment
                              ActiveStorage::Blob
...
Rails.application
     .config
     .middleware
     .insert_after Middleware::SiteInterceptor,
                   Apartment::Elevators::Generic, lambda { |request|
                                                    if request.env['PATH_INFO'].include?('backoffice')
                                                      user = request.env['warden'].user
                                                      return nil unless user&.active?

                                                      if user&.organizations?
                                                        user.principal_organization.tenant_name
                                                      elsif request.session[:organization_id]
                                                        Organization.where(id: request.session[:organization_id])
                                                                    .select(:code)
                                                                    .first
                                                                    .tenant_name
                                                      end
                                                    elsif request.session[:site_organization_id]
                                                      Organization.where(id: request.session[:site_organization_id])
                                                                  .select(:code)
                                                                  .first
                                                                  .tenant_name
                                                    end
